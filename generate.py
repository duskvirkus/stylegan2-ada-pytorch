...
def images(G, device, inputs, space, truncation_psi, label, noise_mode, outdir, save_vectors, start=None, stop=None):
    ...
    for idx, i in enumerate(inputs):
        ...
        if space == 'z':
            if save_vectors:
                np.save(f'{outdir}/vectors/frame{idx:04d}-z', i)
            ...
            if save_vectors:
                np.save(f'{outdir}/vectors/frame{idx:04d}-w', i)
            if i.shape[0] == 18:
...
@click.option('--frames', type=int,
              help='how many frames to produce (with seeds this is frames between each step, with loops this is total '
                   'length)',
...
def generate_images(
    ...
    if process == 'image':
        ...
        for seed_idx, seed in enumerate(seeds):
            ...
            vec = np.random.RandomState(seed).randn(1, G.z_dim)
            if space == 'z':
                if save_vectors:
                ...
                w = G.mapping(z, label, truncation_psi=truncation_psi, truncation_cutoff=8)
                if save_vectors:
                    np.save(f'{outdir}/vectors/seed{seed:04d}-w', w.cpu().detach().numpy())
